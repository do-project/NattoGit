...
page.on("back", function(){
	...
	order_http.url = $U.token($U.url.takeShowOrder,"user")+"&orderno="+Euc(orderno); // 请求的 URL
	order_http.request();
...
order_http.on("success", function(databus) {
	...
	if(databus==-1){
		...
		dtgird.refreshItems();
		deviceone.payMentType = databus.payMent;
		if(deviceone.price==0 || databus.payMent==0){
			var _panel1 = ui("do_ALayout_8");
...
btnSubmit.on("touch",function(){
	...
	submit_http.url = $U.token($U.url.TakeTotalOrderZero,"user")+"&orderno="+Euc(orderno)+"&balance="+Euc(deviceone.useBalance)+"&payType="+deviceone.payment+"&totalNo="+deviceone.totalNo+"&spbillIP="+network.getIP(); // 请求的 URL
	submit_http.request();
});
