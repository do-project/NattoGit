...
camerabtn.on("touch","",300,function(){
	this.animate(buttonA,function(){
		...
		paneler.animate(animPanelHide);
		camera.capture("", "", 100, false, function(data) {
			open.start("source://view/other/imageCrop.ui",data);
	    });
	});
});
//相册选择
...
photobtn.on("touch","",300,function(){
	this.animate(buttonA,function(){
		...
		paneler.animate(animPanelHide);
		album.select(1, "", "", 100, false, function(data, e) {
			open.start("source://view/other/imageCrop.ui",data.toString());
		});
	});
});
