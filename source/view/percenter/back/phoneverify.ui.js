...
page.on("back", function(){
	...
	var _time=global.getMemory("forgetTime");
	if(_time!=""){
...
_verify.on("touch","",300,function(){
	if(_mobileSelect!=_phone.text){
		...
		request_http.url = $U.url.appForgetPassVerifyCode+"?token="+Euc(token)+"&authCode="+_code.text.trim()+"&type=6";
...
_send.on("touch",function(){  //发送验证码
	if($U.regular.phone.test(_phone.text)){
		...
		send_http.url = $U.url.appForgetPassSendCode+"?phone="+_phone.text+"&type=6&Template=1528";
		send_http.request();
	}else{
		nf.toast("请输入正确的手机号码");
...
send_http.on("success", function(databus) {
	if(databus.error_code==0){
		...
		token = databus.token;
		global.setMemory("forgetTime", new Date());  //记录当前时间
		_send.enabled=false;
		timing=time;
		_send.text=timing+"秒后重新发送";
		mTimer.start();  //开始倒计时
...
request_http.on("success", function(databus) {
	if(databus.error_code==0){
		open.start("source://view/percenter/back/safePassword.ui",token);
	}else{
		nf.toast(databus.reason);
		_code.setFocus(true);
	}
