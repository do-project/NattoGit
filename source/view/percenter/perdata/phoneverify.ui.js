...
page.on("back", function(){
	...
	_phone.text=phone;
	var _time=global.getMemory("bindTime");
	if(_time!=""){
...
_verify.on("touch","",300,function(){
	if(_code.text.trim()!=""){
		request_http.url = $U.token($U.url.appVerifyPhone,"user")+"&authCode="+_code.text.trim()+"&type=3&mobile=";
...
_send.on("touch",function(){  //发送验证码
	send_http.url = $U.token($U.url.appBindingPhone,"user")+"&type=3&Template=1541&mobile=";
	send_http.request();
});
...
send_http.on("success", function(databus) {
	if(databus.error_code==0){
		nf.toast("发送成功");
		global.setMemory("bindTime", new Date());  //记录当前时间
		_send.enabled=false;
		timing=time;
		_send.text=timing+"秒后重新发送";
		mTimer.start();  //开始倒计时
...
request_http.on("success", function(databus) {
	if(databus.error_code==0){
		open.start("source://view/percenter/perdata/phoneverify1.ui",2);  //去绑定手机
	}else{
		nf.toast("验证码输入有误");
		_code.setFocus(true);
	}
