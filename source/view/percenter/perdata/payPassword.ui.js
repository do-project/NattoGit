...
var oldPass=ui("do_TextField_1");  //旧密码
...
submit.on("touch","",300,function(){
	if(newPass.text.trim()=="" || newPass.text.length<6){
		nf.toast("新密码长度必须大于6位字符");
		newPass.setFocus(true);
	}else if(newQPass.text!=newPass.text){
		nf.toast("两次密码输入不一致");
		newQPass.setFocus(true);
	}else{
		request_http.url = $U.token($U.url.appUpdatePayPass,"user")+"&oldPass="+Euc(oldPass.text)+"&newPass="+Euc(newPass.text);
