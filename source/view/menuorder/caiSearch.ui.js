...
page.on("loaded",function(){
	hdbgs.animate(animHDdown,function(){
		textf.setFocus(true);
	});
});
...
textf.on("textChanged",function(){
	if(this.text==""){
		...
		searchData.removeAll();
		var _databus=deviceone.param.foodList;
		var databus=[];
		if(deviceone.param.type==0){
			...
			for(var i=0;i<databus.length;i++){
				...
				searchData.updateOne(i, {
					...
					"price":parseFloat(databus[i].price)+"元/例",
			...
			for(var i=0;i<_databus.length;i++){
				if(algorithm.base64Sync("decode", _databus[i].title).indexOf(this.text)!=-1){
					_databus[i].index=i;
					databus[databus.length]=_databus[i];
				}
			}
			...
			for(var i=0;i<databus.length;i++){
				...
				searchData.updateOne(i, {
					"token":databus[i].token,
					"path":databus[i].path,
					"title":algorithm.base64Sync("decode", _databus[i].title),
					"price":parseFloat(databus[i].price)+"元/例",
					"sales":"已售 : "+databus[i].sales+"份",
					"note":databus[i].note,
					"classToken":databus[i].classToken,
					"index":databus[i].index,
					"note":algorithm.base64Sync("decode", databus[i].note),
					"count":_num,
					"show":_num>0,
					"BigPath":databus[i].BigPath,
					"type":6
...
//	var json = {path:cell.path,title:cell.title,sales:cell.sales,price:cell.price,note:cell.note,selected:cell.count>0,index:cell.index,type:deviceone.param.type};
...
function getMenuNum(_token){
	var _num=0;
	for(var i=0;i<deviceone.param.orderList.length;i++){
		var _data = deviceone.param.orderList[i];
